...
export interface PreviousRoundSnapshot {
  result: Result;
  players: Pick<PlayerState, "id" | "name" | "camp">[];
}
...
function greedyComboAIDecision(
  ...
  console.log(`🤖 AI ${player.name} 生成了 ${options.length} 个候选出牌`);
  ...
  console.log(
    `🤖 AI ${player.name} 尝试压住 ${current.type}, mainValue=${current.mainValue}`
  );

  // 显示前5个候选牌型
  console.log(
    `🤖 AI ${player.name} 候选牌型(前5个):`,
    options
      .slice(0, 5)
      .map((opt) => `${opt.type}(${opt.mainValue})`)
      .join(", ")
  );

  const beating = options.filter((opt) => {
    const result = canBeat(current, opt);
    if (!result && opt.type === current.type) {
      console.log(
        `🤖 AI 检查: ${opt.type}(${opt.mainValue}) vs ${current.type}(${current.mainValue}) = ${result}`
      );
    }
    return result;
  });

  console.log(`🤖 AI ${player.name} 找到 ${beating.length} 个可以压住的牌型`);
  ...
  if (beating.length === 0) {
    console.log(`🤖 AI ${player.name} 没有可以压住的牌，选择PASS`);
    return null;
  }
  ...
  const chosen = sortedPool[0]?.cards ?? null;

  if (chosen) {
    const chosenPlay = classifyPlay(chosen);
    console.log(
      `🤖 AI ${player.name} 决定出 ${chosenPlay?.type}:`,
...
export class GameEngine {
  ...
  playAutoTurn(): void {
    ...
    if (!chosen || chosen.length === 0) {
      ...
      if (
        ...
        console.log(
          `⚪ 本轮其他玩家全部 PASS，本墩结束并清空桌面。最后出牌: ${
            s.players[s.lastPlayOwnerIndex].name
          }`
        );
        ...
        console.log(`🔄 新一轮开始，由 ${s.players[newStarter].name} 首家出牌`);
      } else {
  ...
  private executePlay(playerIndex: number, cards: Card[]): void {
    ...
    if (s.lastPlay && !canBeat(s.lastPlay, play)) {
      console.log(
        `❌ ${player.name} 出的牌 ${cards.map(formatCard).join(" ")} (${
          play.type
        }) 无法压住桌面 ${s.lastPlay.type}，只能 PASS`
      );
      s.passesInRound += 1;
      s.currentPlayerIndex = nextActiveIndex(s, playerIndex);
      return;
    ...
    if (s.lastPlay) {
      console.log(
        `✅ ${player.name} 出牌：${cards.map(formatCard).join(" ")} (${
          play.type
        }) 压住了 ${s.lastPlay.type}`
      );
    } else {
      console.log(
        `✅ ${player.name} 首家出牌：${cards.map(formatCard).join(" ")} (${
          play.type
        })`
      );
    }
    ...
    console.log(
      `📋 桌面牌型已更新: ${s.lastPlay.type}, mainValue=${s.lastPlay.mainValue}`
    );
  ...
  private askOthersWhetherToBeat(finishedIndex: number): void {
    ...
    for (const i of askOrder) {
      ...
      const chosen = this.decideFor(i, context);
  ...
  private settle(): void {
    ...
    s.result = result; // ✅ 同时保存到 state 中，供前端UI使用
...
export interface CreateGameOptions {
  ...
  startPlayerIndex?: number; // 指定起始玩家，默认为0（P1）
}
...
export function createInitialGame(
  ...
  const { controllers = {}, playerNames = [], startPlayerIndex = 0 } = options;
  ...
  // 使用指定的起始玩家索引，默认为0（P1总是第一个出）
  const firstIndex = startPlayerIndex;
